...
[Service]
Environment="KUBELET_EXTRA_ARGS=--enable-controller-attach-detach=false --cloud-provider=aws --runtime-cgroups=/systemd/system.slice --kubelet-cgroups=/systemd/system.slice --authentication-token-webhook --node-labels=node-role.kubernetes.io/master="
EOF
